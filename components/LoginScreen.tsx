...
export default NiceModal.create(() => {
  ...
  const handleLogin = async (email: string) => {
    try {
      ...
      setMessage({level: 'success', text: 'Check your email for a login link'});
    } catch (error: any) {
      setMessage({level: 'error', text: error.error_description || error.message});
    } finally {
      setLoading(false)
    }
  };
      ...
      {message?.level && message?.text && (
        <>
          ...
          <div className="text-right mt-3">
            ...
            <button className="hover:text-yellow-500 px-3" onClick={modal.remove}>
              close
            </button>
          </div>
        </>
